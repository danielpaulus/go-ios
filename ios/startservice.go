...
//StartServiceResponse is sent by the phone after starting a restapi, it contains servicename, port and tells us
...
func (lockDownConn *LockDownConnection) StartService(serviceName string) (StartServiceResponse, error) {
	...
	if response.Error != "" {
		return StartServiceResponse{}, fmt.Errorf("Could not start restapi:%s with reason:'%s'. Have you mounted the Developer Image?", serviceName, response.Error)
	}
...
//StartService conveniently starts a restapi on a device and cleans up the used UsbMuxconnection.
//It returns the restapi port as a uint16 in BigEndian byte order.
