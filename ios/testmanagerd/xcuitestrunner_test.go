...
func TestXcuiTest(t *testing.T) {
	...
	var wdaenv []string
	go func() {
		_, err := testmanagerd.RunXCUIWithBundleIdsCtx(ctx, bundleID, testbundleID, xctestconfig, device, wdaargs, wdaenv, testmanagerd.NewTestListener(os.Stdout, os.Stdout))
		if err != nil {
